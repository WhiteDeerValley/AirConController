...
class MasterMachine:
    ...
    def get_report(self, room_id: str, start_time: datetime.datetime, finish_time: datetime.datetime):
        ...
        last_aim = logs[0].aim
        last_speed = logs[0].speed
        last_time = logs[0].op_time
        last_op = logs[0].operation
        dict_aim = {}
        dict_speed = {}
        for l in logs[1:]:
            #如果上一次操作不是关机则数据有效
            if last_op != operations.POWER_OFF:
                if last_aim in dict_aim:
                    dict_aim[last_aim] = dict_aim[last_aim] +(l.op_time - last_time).seconds
                else:
                    dict_aim[last_aim] = (l.op_time - last_time).seconds
                if last_speed in dict_speed:
                    dict_speed[last_speed] =dict_speed[last_speed] + (l.op_time - last_time).seconds
                else:
                    dict_speed[last_speed] = (l.op_time - last_time).seconds
            #不管上一个操作是不是关机，都要更新上一操作的操作时间
            last_aim = l.aim
            last_speed = l.speed
            last_time = l.op_time
            last_op = l.operation
        ...
        if last_op != operations.POWER_OFF:
            # 如果是同一天
            if last_time.strftime("%Y-%m-%d") == now.strftime("%Y-%m-%d"):
                if last_aim in dict_aim:
                    dict_aim[last_aim] = dict_aim[last_aim] +(now - last_time).seconds
                else:
                    dict_aim[last_aim] = (now - last_time).seconds
                if last_speed in dict_speed:
                    dict_speed[last_speed] = dict_speed[last_speed] + (now - last_time).seconds
                else:
                    dict_speed[last_speed] = (now - last_time).seconds
            # 如果不是同一天
            else:
                last_begin = datetime.datetime(last_time.year, last_time.month, last_time.day, 0, 0, 0)
                last_tomorrow_begin = last_begin + datetime.timedelta(days=1)
                rest_seconds = (last_tomorrow_begin - now).seconds
                if last_aim in dict_aim:

                    dict_aim[last_aim] = dict_aim[last_aim] + rest_seconds
                else:
                    dict_aim[last_aim ] = dict_aim[last_aim] + rest_seconds
                if last_speed in dict_speed:

                    dict_speed[last_aim] = dict_speed[last_aim] + rest_seconds
                else:
                    dict_speed[last_aim] = dict_speed[last_aim] + rest_seconds

        if dict_aim:
            usually_aim = max(dict_aim,key=dict_aim.get)
        else:
            usually_aim = 0
        if dict_speed:
            usually_speed = max(dict_speed,key=dict_aim.get)
        else:
            usually_speed = 0

        return Report(room_id, start_time, finish_time, duration, times_of_on_off, times_of_dispatch,
                      usually_aim, usually_speed, len(details), round(fee, 2))
...
class Report:
    ...
    Attributes:
        ...
        __usually_aim:最常使用目标温度
        __usually_speed:最长使用风速
        __number_of_detail: 详单条目数
        __fee: 总费用
        __times_of_get_aim:达到目标温度次数
    """

    def __init__(self, room_id, start_time, finish_time, duration, times_of_on_off, times_of_dispatch,
                 usually_aim, usually_speed, number_of_detail, fee):
        ...
        self.__usually_aim = usually_aim
        self.__usually_speed = usually_speed
    ...
    @property
    def usually_aim(self):
        return self.__usually_aim

    @property
    def usually_speed(self):
        return self.__usually_speed
